...
## (g) Generate MCP Token — `/dashboard/generate-token`

Once signed in, operators can mint a new MCP token on demand from the dashboard.

### Flow

```
GET /dashboard (authenticated)
   ↓ server renders form with ch_csrf hidden field
Click "Generate MCP Token"
   ↓ POST /dashboard/generate-token (csrf_token form field + ch_csrf cookie)
Server validates CSRF (double-submit cookie pattern)
   ↓ session check → CSRF check → auth.IssueMCPToken(sub, email)
   ↓ re-renders /dashboard with token in one-shot panel
User copies the token → pastes into ~/.claude.json
   ↓ subsequent GETs to /dashboard show empty state (token NOT stored)
```

### CSRF — double-submit cookie pattern

Both endpoints that mutate state (`POST /dashboard/generate-token` and `POST /auth/logout`) are protected by the double-submit cookie pattern:

1. On `GET /dashboard`, if the `ch_csrf` cookie is absent, the server generates a 32-byte random hex token, sets `Set-Cookie: ch_csrf=<hex>; Path=/; SameSite=Strict; HttpOnly=false; Max-Age=86400; Secure={conditional}`, and injects it into both forms as a hidden `<input name="csrf_token">`.
2. On POST, the server reads both the `ch_csrf` cookie value and the `csrf_token` form field and compares them with `subtle.ConstantTimeCompare`. Mismatch or missing either field → `403 Forbidden` (no token is issued).

`ch_csrf` is deliberately NOT `HttpOnly` — the template renders its value into the HTML on `GET /dashboard`, so the value is always server-sourced, not client-readable after page load in a meaningful way. The `SameSite=Strict` attribute prevents cross-site forgery from using the cookie at all, providing belt-and-suspenders protection.

### Token properties

The token generated by `POST /dashboard/generate-token` is **bit-identical** in shape to the one issued by `POST /auth/exchange`:

| Property | Value |
|---|---|
| Algorithm | HS256 |
| Claims | `sub` (Supabase user UUID), `iat`, `exp`, `email`, `iss` |
| Expiry | 1 year (or `MCP_JWT_EXPIRY` env var) |
| Issuer | `context-harness-mcp` (or `MCP_JWT_ISSUER` env var) |

Multiple tokens remain valid simultaneously — **no prior token is invalidated when a new one is generated** (locked decision: multi-token model). Revocation is user-level via `users.revoked_at`.

### One-shot display

The generated token is shown once in a `<code>` block with a "Copy" button and a warning "Save this now — it won't be shown again." The token is **not stored server-side**. Subsequent `GET /dashboard` requests show the empty generate-token form.
