...
- **Viewer requires authentication (OVERRIDE of v0.2.0 lock).** `/viewer/*` requires a valid `ch_session` cookie. HTML paths (`/viewer/`, non-api) → `302` to `/auth/login?next=<path>`. JSON API paths (`/viewer/api/*`) → `401 {"error":"authentication required"}`. Ratified at STAGE-GATE-1 of the auth-ux-redesign feature (see `session-docs/auth-ux-redesign/00-task-intake.md` AC-9).
- **Web-session pattern.** An HMAC-SHA256 signed `ch_session` cookie (24h expiry, `HttpOnly`, `SameSite=Strict`) is issued alongside the MCP JWT on every successful `/auth/exchange`. The cookie is distinct from the MCP JWT: it gates browser surfaces (`/dashboard`, `/viewer/*`) while the JWT gates `/mcp/*`. Both are signed with `MCP_JWT_SECRET` but use distinct HMAC inputs (domain-separation suffix `\x00ch_session_v1`). Session state lives exclusively in the cookie — no DB table.
...
|---------|------------------------|-----------------|
| Architecture Decisions | 10 | `docs/decisions.md` |
| Patterns & Conventions | 10 | `docs/patterns.md` |
| Known Constraints | 10 | `docs/constraints.md` |
| Testing Conventions | 10 | `docs/testing.md` |
