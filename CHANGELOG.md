...
- `feat(validate): SortedNodeTypes() helper` — `internal/validate/taxonomy.go`: exports `SortedNodeTypes() []string` returning the NodeTypes map keys in alphabetical order. Used by the viewer handler for the static node-types API response.
